    ...
    for (i in c(250,550,850)){
      for (t in  c("g0 tilde bk", "g0 tilde 1", "g0 tilde bk + t", "g0 tilde t")){
          ...
          for (u in 1:(nrow(compiled))){
              ...
              newLine<-nextSim
              newLine$DenhatBiasFull<-nextSim[,4]-trueDensity #Universal secr density is animals per hectare
...
ggplot(data=filter(discrep, whichSource=="SimData"))+
  scale_fill_grey() +
  geom_boxplot(aes(x=trial, y=DenhatBiasFull, fill=SubsamplingType)) +
  ylab("d-hat bias") +
...
discrep <- read.csv("Checks/discrep.csv")
discrep <- mutate(discrep, DenhatBias_param = Denhat / trueDensity)
#Money plots

ggplot(data=filter(compiled, trial==c("t4","t5","t6","t7", "Empirical"), subtype!="Full"))+
...
ggplot(data=filter(compiled, trial==c("t4","t5","t6","t7"), subtype!="Full", SubProp.notRedun>0))+
  scale_fill_grey(name="Subsampling\nType") +
  geom_boxplot(aes(x=trial, y= (1 - SubProp.notRedun), fill=subtype)) +
  ylab("Proportion of \nRedundant Samples") +
  xlab("Trial") +
  ggtitle("Sample Redundancy \nvs Subsampling Type") +
  theme_grey(14) +
...
ggplot(data=filter(discrep, whichSource=="SimData"))+
  scale_fill_hue() +
  geom_boxplot(aes(x=trial, y=DenhatBias_param, fill=subtype)) +
  ylab("Derived density") +
  xlab("Trial") +
  ggtitle("d-hat \nvs Subsampling Type (Simulated)") +
  theme_light() +
...
ggplot(data=filter(discrep, whichSource=="SimData", size==250, model %in% c("g0 ~ 1", "g0 ~ bk")))+
  geom_boxplot(aes(x=trial, y=DenhatBias_param, fill=subtype)) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $D$ (Simulated, n=250)')) +
  ylab("Derived density / True density") +
  xlab("Scenario") +
  theme_grey(14) +
  scale_fill_grey(name="Subsampling\nType") +
  facet_wrap(~model, ncol=2)+
  geom_hline(yintercept=1, linetype =2)  + ggsave("Figures\\Dsub_D_simmed.png", height=5, width = 7.5)
...
discrep2 <- discrep %>% 
  filter(whichSource!='EmpData')%>%
  group_by(subtype, trial, model, size) %>%
  summarise(meanDenhatFullPct = mean(DenhatBiasFullPct), CI_denhatBias = 1.96*sd(DenhatBiasFullPct)/sqrt(n()),
            meanDenhatTruePct = mean(DenhatBias_param), CI_denhatTrueBias = 1.96*sd(DenhatBias_param)/sqrt(n()))

ggplot(data=filter(discrep2, subtype!='Full', trial!="Empirical", size==250, model %in% c("g0 ~ 1", "g0 ~ bk")), aes(color = subtype,x=trial, group = subtype))+
  geom_point(aes(y=meanDenhatFullPct, shape=subtype), position=position_dodge(.4), size=3, fill="white", stat = 'identity') +
...
ggplot(data=filter(discrep2, subtype!='Full',trial!="Empirical", model %in% c("g0 ~ 1", "g0 ~ bk")), aes(color = subtype,x=trial, group = subtype))+
  geom_point(aes(y=meanDenhatFullPct, shape=subtype), position=position_dodge(.4), size=3, fill="white", stat = 'identity') +
  geom_errorbar(aes(ymin=meanDenhatFullPct - CI_denhatBias, ymax=meanDenhatFullPct + CI_denhatBias), position=position_dodge(.4), colour="black", width = .4) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $\\hat{D}_{Full}$ (Simulated, n=250)')) +
  ylab("Derived density of subsample / \nDerived density of full sample (95% CI)") +
...
ggplot(data=filter(discrep2, trial!="Empirical", size==250, model %in% c("g0 ~ 1", "g0 ~ bk")), aes(color = subtype,x=trial, group = subtype))+
  geom_point(aes(y=meanDenhatTruePct, shape=subtype), position=position_dodge(.4), size=3, fill="white", stat = 'identity') +
...
ggplot(data=filter(discrep2, trial!="Empirical", subtype!='Full', size==250, model %in% c("g0 ~ 1", "g0 ~ bk")), aes(color = subtype,x=trial, group = subtype))+
  geom_point(aes(y=meanDenhatTruePct, shape=subtype), position=position_dodge(.4), size=3, fill="white", stat = 'identity') +
  geom_errorbar(aes(ymin=meanDenhatTruePct - CI_denhatTrueBias, ymax=meanDenhatTruePct + CI_denhatTrueBias), position=position_dodge(.4), colour="black", width = .4) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $D$ (Simulated, n=250)')) +
  ylab("Derived density / True density (95% CI)") +
...
ggplot(data=filter(discrep2,trial!="Empirical", model %in% c("g0 ~ 1", "g0 ~ bk")), aes(color = subtype,x=trial, group = subtype))+
  geom_point(aes(y=meanDenhatTruePct, shape=subtype), position=position_dodge(.4), size=3, fill="white", stat = 'identity') +
  geom_errorbar(aes(ymin=meanDenhatTruePct - CI_denhatTrueBias, ymax=meanDenhatTruePct + CI_denhatTrueBias), position=position_dodge(.4), colour="black", width = .4) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $D$ (Simulated)')) +
  ylab("Derived density / True density (95% CI)") +
  xlab("Scenario") +
  theme_grey(14) +
  scale_color_manual(values= c('seagreen','dodgerblue1', 'dodgerblue4')) +
  facet_grid(size~model)+
  theme(legend.position="bottom", legend.title=element_blank()) +
  geom_hline(yintercept=1, linetype =2)  + ggsave("Figures\\pointsCI_ALL_Dsub_D_simmed.png", height=8, width = 7.5)


ggplot(data=filter(discrep, whichSource=="SimData", size==250, model %in% c("g0 ~ 1", "g0 ~ bk"), subtype!="Full"))+
  geom_boxplot(aes(x=trial, y=DenhatBias_param, fill=subtype)) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $D$ (Simulated, n=250)')) +
  ylab("Derived density / True density") +
  xlab("Scenario") +
  theme_grey(14) +
  facet_wrap(~model, ncol=2)+
  geom_hline(yintercept=1, linetype =2)+ ggsave("Figures\\Dsub_D_simmed_noFull.png", height=5, width = 7.5)
...
ggplot(data=filter(discrep, whichSource=="SimData", size!=850))+
  scale_fill_grey(name="Subsampling\nType") +
  geom_boxplot(aes(x=trial, y=plogis(g0 + g0.bTRUE), fill=SubsamplingType)) +
  ylab("g0b") +
  ggtitle("g0b vs Subsampling Type (Simulated)") +
  theme_grey(14) +
  facet_wrap(size~model, ncol=4) +
