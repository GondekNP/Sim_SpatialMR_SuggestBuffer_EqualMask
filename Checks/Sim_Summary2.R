...
PropNotRedun_Sim <- ggplot(data=filter(compiled, trial==c("t4","t5","t6","t7"), subtype!="Full"))+
  scale_fill_grey(name="Subsampling\nType") +
  geom_boxplot(aes(x=trial, y=SubProp.notRedun, fill=subtype)) +
  ylab("Proportion of \nnon-redundant Samples") +
  xlab("Scenario") +
  ggtitle("Sample Redundancy vs Subsampling Type (Simulated)") +
  theme_grey(14) +
  facet_wrap(~size)
ggsave(PropNotRedun_Sim, "Figures/PropNonRedun_Sim")

ggplot(data=filter(discrep, whichSource=="SimData", size=="250", model == "g0 tilde 1" | model == "g0 tilde bk", subtype!="Full", DenhatBiasFullPct > .5 & DenhatBiasFullPct < 1.4))+
  scale_fill_grey(name="Subsampling\nType") +
  geom_boxplot(aes(x=trial, y=DenhatBiasFullPct, fill=SubsamplingType)) +
  ylab(TeX('$\\hat{D}_{Sub}$ / $\\hat{D}_{Full}$')) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $\\hat{D}_{Full}$ \n vs Subsampling Type and Simulation Scenario (Simulated, n=250)')) +
  theme_grey(14) +
  geom_hline(aes(yintercept=1), linetype='dashed')+
  facet_wrap(~str_replace(string = model, "tilde", "\\~"), ncol=4)

ggplot(data=filter(discrep, whichSource=="EmpData", subtype!="Full"))+
  scale_fill_grey(name="Subsampling\nType") +
  geom_boxplot(aes(x = as.character(size), y=DenhatBiasFullPct, fill=SubsamplingType)) +
  ylab(TeX('$\\hat{D}_{Sub}$ / $\\hat{D}_{Full}$')) +
  ggtitle(TeX('$\\hat{D}_{Sub}$ / $\\hat{D}_{Full}$ \n vs Subsampling Type and Simulation Scenario (Empirical)')) +
  theme_grey(14) +
  geom_hline(aes(yintercept=1), linetype='dashed')+
  facet_wrap(~str_replace(string = model, "tilde", "\\~"), ncol=4) +
  xlab("Size of Subsample")
